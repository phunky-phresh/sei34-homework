...
function recursiveFlood(x, y, targetValue, newValue){
    ...
    // check all 9 adjacent cell for same colour as original colour
    // change color to new color
    // rerun recursiveFlood() with that cell as new x,y

    if (targetValue === newValue) {
        // skip if no colour change
    } else {
        let currentNode = $(`div[x="${x}"][y="${y}"]`); // pull the targetted node, and update its color
        currentNode.attr('num', newValue);

        // I have assumed that adjacent here includes diagonally adjacence 
        for (let row = y - 1; row <= y + 1; row ++) { // find the row above and below, and the cells above and below
            for (let col = x - 1; col <= x + 1; col ++) {
                // can ignore double checking center cell, since its colour will have changed already above
                let node = $(`div[x="${col}"][y="${row}"]`); // pull the node for those 9 cells, one by one
                if ( parseInt(node.attr('num')) === targetValue) { // if the cell tested is the same colour as the original cell
                    currentNode.attr('num', newValue); // update the cell
                    recursiveFlood(col, row, targetValue, newValue); // call the recursion again with same colours but new centre cell
                }
            }
        }
    }
