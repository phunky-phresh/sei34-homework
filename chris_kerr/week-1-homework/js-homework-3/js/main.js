...
function maxOfThreeNumbers(input1, input2, input3) {
  let interim1 = maxOfTwoNumbers(input1, input2);
  return maxOfTwoNumbers(interim1, input3);
}
...
function multiplyArray(inputArray) {
  let runningMult = 1;
  for (let i = 0; i < inputArray.length; i++) {
    runningMult = runningMult * inputArray[i];
  }
  return runningMult;
}
...
function findLongestWord(input) {
  ...
  for (let i = 0; i < input.length; i++) {
    if (input[i].length > longestFound) longestFound = input[i].length;
  }
  return longestFound;
}

function filterLongWords(input, length) {
  let filteredInput = [];
  for (let i = 0; i < input.length; i++) {
    if (input[i].length > length) filteredInput.push(input[i]);
  }
...
function reward(input) {
  if (input === "correct") rewardTally = rewardTally + Math.random()*10;
  if (input === "wrong") rewardTally = rewardTally - Math.random()*10;
  rewardTally = Number(rewardTally.toFixed(2));
  //console.log(rewardTally);
}
