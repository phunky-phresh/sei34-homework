...
def plan_trip_one_line(line, a, b)
  ...
  if $line[line].index(a) < $line[line].index(b)
    stops = $line[line][($line[line].index(a) + 1)..$line[line].index(b)]
    $number_of_stops += stops.size
  elsif $line[line].index(a) > $line[line].index(b)
    stops = $line[line][$line[line].index(b)..($line[line].index(a) - 1)].reverse
    $number_of_stops += stops.size
  end
  return stops.join(", ")
end
...
def plan_trip(start_line, start_stop, end_line, end_stop)
  ...
  if start_line == end_line
    ...
    puts "Total of #{ $number_of_stops } stops."
  end
end
