...
$(document).ready(function(){
    ...
    if (showPlant) {
        $(".plant_hidden_show").slideDown();
    }
    if (showPest) {
        $(".pest_hidden_show").slideDown();
    }
    ...
    $(".part_plant").click(() => {
        showPlant = true;
        localStorage.setItem("showPlant", JSON.stringify(showPlant))})
    $("button.plant_hide_show").click(()=> {
        showPlant = false;
        localStorage.setItem("showPlant", JSON.stringify(showPlant))})
    $(".show_plant_pest").click(() => {
        showPlant = false; localStorage.setItem("showPlant", JSON.stringify(showPlant));
        showPest = true; localStorage.setItem("showPest", JSON.stringify(showPest))})


        $(".part_pest").click(() => {
            showPest = true;
            localStorage.setItem("showPest", JSON.stringify(showPest))
        })
        $("button.pest_hide_show").click(()=> {
            showPest = false;
            localStorage.setItem("showPest", JSON.stringify(showPest));})
        $(".show_pest_plant").click(() => {
            ...
            $(".delete_button_plant").click(() => { showPlant = false;
            localStorage.setItem("showPlant", JSON.stringify(showPlant));})
            $(".delete_button_pest").click(() => { showPest = false; $localStorage.setItem("showPlant", JSON.stringify(showPlant));})
